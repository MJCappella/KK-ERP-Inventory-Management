...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="page-title">Wholesale Products & SKUs</h1>
            <p class="page-subtitle">Manage wholesale product master catalog, prices, reorder levels, and store inventory distributions.</p>
        </div>
        @if(auth()->user()->isAdmin())
            <div>
                <a href="{{ route('products.create') }}" class="btn btn-primary shadow-sm shadow-sky-600/30">
    ...
    <div class="filter-card">
        <form method="GET" action="{{ route('products.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
            <div>
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search name or SKU..." class="form-input text-xs">
            </div>
                ...
                <tbody>
                    @forelse($products as $p)
                        <tr>
                            ...
                            <td>
                                <span class="font-bold text-sm {{ $p->total_stock <= $p->reorder_level ? 'text-amber-600' : 'text-slate-900' }}">
                                    {{ $p->total_stock }}
                                </span>
                            </td>
                            <td>
                                <div class="flex flex-wrap gap-1">
                                    @foreach($p->storeStocks as $stk)
                                        <span class="text-[10px] px-2 py-0.5 rounded bg-slate-100 font-medium text-slate-700" title="{{ $stk->store->name }}">
                            ...
                            @if(auth()->user()->isAdmin())
                                <td>
                                    <div class="flex items-center gap-1.5">
                                        <a href="{{ route('products.edit', $p) }}" class="btn btn-secondary btn-sm">
                                            Edit
                                        </a>
                                    </div>
                                </td>
