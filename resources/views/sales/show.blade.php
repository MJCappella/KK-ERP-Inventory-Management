...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between no-print">
            <a href="{{ route('sales.index') }}" class="btn btn-primary btn-sm flex items-center gap-1.5">
                <i class="fa-solid fa-arrow-left text-xs"></i>
                <span>Back to Sales History</span>
            </a>
            <div class="flex items-center gap-2">
                <button type="button" onclick="window.print()" class="btn btn-primary btn-sm flex items-center gap-1.5">
                    <i class="fa-solid fa-print text-xs"></i>
                    <span>Print Invoice / Receipt</span>
                </button>
                <a href="{{ route('sales.pos') }}" class="btn btn-secondary btn-sm flex items-center gap-1.5">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-md p-8 md:p-10">
            ...
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between border-b border-slate-200 pb-8 gap-6">
                <div>
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500 font-semibold">{{ $sale->store->name }} &bull;
                                {{ $sale->store->branch->name }}</p>
                        </div>
                    </div>
                ...
                <div class="sm:text-right">
                    <span
                        class="inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-emerald-100 text-emerald-800 mb-2">
                    ...
                    <p class="text-xs text-slate-500 mt-1">Date: <span
                            class="font-mono font-medium text-slate-700">{{ $sale->created_at->format('M d, Y H:i:s') }}</span>
                    </p>
                    <p class="text-xs text-slate-500">Cashier: <strong
                            class="text-slate-700">{{ $sale->user->name }}</strong></p>
                </div>
            </div>
            ...
            <div class="py-6 flex justify-end">
                <div class="w-full sm:w-72 space-y-2 text-xs">
                    ...
                    @endif
                    <div
                        class="pt-2 border-t border-slate-200 flex justify-between font-extrabold text-base text-slate-900">
            ...
            <div
                class="bg-slate-50 rounded-xl p-4 border border-slate-200/80 text-[11px] text-slate-500 flex flex-col sm:flex-row items-center justify-between gap-2">
                <div>
                    <strong class="text-slate-700">Stock Ledger Audit Status:</strong> Automatically recorded in <code
                        class="font-mono text-sky-700">stock_movements</code> with row-level locks.
                </div>
                <div>
                    <a href="{{ route('movements.index', ['search' => $sale->invoice_number]) }}"
