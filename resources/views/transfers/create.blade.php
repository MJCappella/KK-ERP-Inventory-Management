...
<div x-data="transferForm()" class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="page-subtitle">Atomically deduct items from origin store and credit destination store with double-entry ledger logging.</p>
        </div>
    ...
    <form method="POST" action="{{ route('transfers.store') }}" @submit="handleSubmit($event)" class="space-y-6">
        ...
        <div class="card">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Source Store -->
                <div>
                    <label for="source_store_id" class="filter-label">Source Store (Deduct From) *</label>
                    <select id="source_store_id" name="source_store_id" x-model="sourceStoreId" @change="onSourceChange()" required class="form-select text-sm font-semibold text-slate-900">
                    ...
                    <p class="text-[11px] text-slate-500 mt-1">Available inventory will be verified against this store.</p>
                </div>

                <!-- Destination Store -->
                <div>
                    <label for="destination_store_id" class="filter-label">Destination Store (Credit To) *</label>
                    <select id="destination_store_id" name="destination_store_id" x-model="destStoreId" required class="form-select text-sm font-semibold text-slate-900">
        ...
        <div class="card">
            ...
            <div class="p-6 space-y-4">
                                ...
                                <tr>
                                    ...
                                    <td class="py-3">
                                        <span class="font-bold text-xs px-2.5 py-1 rounded-full inline-block"
                                            ...
                                            <span x-text="row.available_stock !== null ? row.available_stock + ' avail' : '—'"></span>
                                        </span>
                                    </td>
                                    <td class="py-3">
                                        <input type="number" 
                                    ...
                                    <td class="py-3 text-center">
                                        <button type="button" @click="removeRow(idx)" class="p-1.5 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded-lg">
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="2" placeholder="e.g. Stock replenishment for weekly retail demand."
            ...
            <div class="card-header bg-slate-50 flex items-center justify-between">
                <span class="text-xs text-slate-500">
                    Total Lines: <strong x-text="rows.length"></strong> &bull; Total Units: <strong x-text="totalUnits"></strong>
                </span>
                <div class="flex items-center gap-3">
                    <a href="{{ route('transfers.index') }}" class="btn btn-secondary btn-sm">Cancel</a>
                    <button type="submit" class="btn btn-primary btn-sm">
                        <span>Dispatch & Execute Transfer</span> &rarr;
                    </button>
                </div>
            </div>
        </div>
