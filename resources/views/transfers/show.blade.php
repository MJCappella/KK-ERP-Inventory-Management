    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-md p-8 md:p-10">
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between border-b border-slate-200 pb-8 gap-6">
            ...
            <div class="sm:text-right">
                ...
                <h2 class="text-2xl font-black font-mono text-slate-900 tracking-tight">{{ $transfer->transfer_number }}</h2>
                <p class="text-xs text-slate-500 mt-1">Date: <span class="font-mono font-medium text-slate-700">{{ $transfer->created_at->format('M d, Y H:i:s') }}</span></p>
                <p class="text-xs text-slate-500">Initiated By: <strong class="text-slate-700">{{ $transfer->user->name }}</strong></p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 py-6 border-b border-slate-200 text-xs">
            ...
            <div class="bg-slate-50 p-4 rounded-xl border border-slate-200/80">
                <p class="font-bold uppercase tracking-wider text-slate-400 mb-1 text-[10px]">Origin (Dispatched From)</p>
            ...
            <div class="bg-sky-50/50 p-4 rounded-xl border border-sky-200/80">
                <p class="font-bold uppercase tracking-wider text-sky-600 mb-1 text-[10px]">Destination (Delivered To)</p>
        ...
        <div class="pt-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-xs">
            ...
            <div>
                <a href="{{ route('movements.index', ['search' => $transfer->transfer_number]) }}" class="text-sky-600 hover:underline font-bold">
                    View Associated Stock Ledger Entries &rarr;
                </a>
            </div>
        </div>
